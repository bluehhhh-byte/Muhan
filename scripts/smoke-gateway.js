...
async function main() {
  const targetServer = net.createServer((socket) => {
    socket.on('error', () => {});
    socket.write(Buffer.from('hello 무한\n', 'utf8'));
    socket.on('data', (chunk) => {
      socket.write(Buffer.from(`echo: ${chunk.toString('utf8')}`, 'utf8'));
    });
  });
  ...
  try {
    ...
    const { socket, reader } = await connectWebSocket(webPort);

    const welcome = await reader.readFrame();
    if (!welcome.payload.toString('utf8').includes('hello 무한')) {
      throw new Error(`unexpected welcome payload: ${welcome.payload.toString('utf8')}`);
    }

    socket.write(encodeClientFrame('look\n'));
    const echo = await reader.readFrame();
    if (!echo.payload.toString('utf8').includes('echo: look')) {
      throw new Error(`unexpected echo payload: ${echo.payload.toString('utf8')}`);
    }
