...
const scarTest = vm.runInNewContext('(() => { const oldScars = character.scars.slice(); const oldMemory = JSON.parse(JSON.stringify(nekoMemory)); const oldMood = character.mood; const oldInsight = character.insight; const oldScene = character.lastScene; character.scars = []; addDefeatScar({ name: "테스트 몬스터", trait: "독" }, "테스트"); const result = [character.scars.length, scarSummary(), effectiveStats().spirit > character.spirit]; character.scars = oldScars; nekoMemory = oldMemory; character.mood = oldMood; character.insight = oldInsight; character.lastScene = oldScene; return result; })()', context);
...
if (savedState.saveVersion !== 9 || savedState.autoStrategy !== '파밍' || !savedState.autoDirector || savedState.frontierPhaseIndex !== 2 || !Array.isArray(savedState.goldLog) || savedState.character.gambleDebt !== 9 || !savedState.character.scars.length || savedState.rogue.fragments !== 2 || !savedState.rogue.perks) throw new Error('저장 버전/경제/파편/전략 상태 저장 실패');
delete storage['muhan.game.state'];
const forcedResetGuard = vm.runInNewContext('(() => { const oldWarning = saveLoadWarning; safeRemoveStorage(RESET_MARKER_KEY); localStorage.setItem(GAME_STATE_KEY, JSON.stringify({ character: { level: 99, gold: 999999 } })); localStorage.setItem(NEKO_MEMORY_KEY, JSON.stringify({ level: 88, exp: 777 })); localStorage.setItem(SETTINGS_KEY, JSON.stringify({ prompt: "유지", level: "9" })); const didReset = resetStoredProgressIfNeeded(); const result = [didReset, localStorage.getItem(GAME_STATE_KEY), localStorage.getItem(NEKO_MEMORY_KEY), localStorage.getItem(SETTINGS_KEY), localStorage.getItem(RESET_MARKER_KEY), saveLoadWarning.includes("리셋")]; saveLoadWarning = oldWarning; return result; })()', context);
if (!forcedResetGuard[0] || forcedResetGuard[1] !== null || forcedResetGuard[2] !== null || !forcedResetGuard[3]?.includes('유지') || forcedResetGuard[4] !== '2026-07-01-full-reset-1' || !forcedResetGuard[5]) throw new Error('강제 진행 리셋 실패');
const inventoryGuard = vm.runInNewContext('(() => { const oldInv = character.inventory.slice(); const oldGold = character.gold; const oldCustom = JSON.parse(JSON.stringify(customItems)); const rare = "전설 흡혈 무기 테스트"; customItems[rare] = { slot: "무기", attack: 99, special: "흡혈", lineage: "테스트 계보" }; character.inventory = Array.from({ length: 260 }, (_, index) => `낡은 전리품${index}`); character.inventory.push(rare); const started = Date.now(); commitProgress(); const elapsed = Date.now() - started; const saved = JSON.parse(localStorage.getItem(GAME_STATE_KEY)); const result = [character.inventory.length, statusPanelEl.textContent.includes("외 "), statusPanelEl.textContent.includes(rare), saved.character.inventory.length, Boolean(saved.customItems[rare]), elapsed]; character.inventory = oldInv; character.gold = oldGold; customItems = oldCustom; return result; })()', context);
if (inventoryGuard[0] > 120 || !inventoryGuard[1] || !inventoryGuard[2] || inventoryGuard[3] > 120 || !inventoryGuard[4] || inventoryGuard[5] >= 500) throw new Error('대형 보관함 Chrome 멈춤 방지 실패');
const oversizedSaveGuard = vm.runInNewContext('(() => { const oldMax = balanceConfig.maxSaveBytes; const oldWarning = saveLoadWarning; balanceConfig.maxSaveBytes = 10; localStorage.setItem(GAME_STATE_KEY, "x".repeat(20)); loadGameState(); const result = [localStorage.getItem(GAME_STATE_KEY), saveLoadWarning.includes("Chrome")]; balanceConfig.maxSaveBytes = oldMax; saveLoadWarning = oldWarning; return result; })()', context);
if (oversizedSaveGuard[0] !== null || !oversizedSaveGuard[1]) throw new Error('과대 저장소 Chrome 멈춤 방지 실패');
const chromeStorageGuard = vm.runInNewContext('(() => { const oldWarning = saveLoadWarning; const oldFields = Object.fromEntries(Object.entries(fields).map(([key, field]) => [key, field.value])); localStorage.setItem(NEKO_MEMORY_KEY, "x".repeat(balanceConfig.maxMemoryBytes + 5)); const memory = loadNekoMemory(); const memoryGone = localStorage.getItem(NEKO_MEMORY_KEY) === null; localStorage.setItem(SETTINGS_KEY, JSON.stringify({ prompt: "가".repeat(5000), level: "999", luck: "999" })); loadSettings(); const settingsClamped = fields.prompt.value.length <= balanceConfig.maxPromptChars && fields.level.value === "99" && fields.luck.value === "99"; localStorage.setItem(SETTINGS_KEY, "x".repeat(balanceConfig.maxSettingsBytes + 5)); loadSettings(); const settingsGone = localStorage.getItem(SETTINGS_KEY) === null; const warned = saveLoadWarning.includes("네코 기억") && saveLoadWarning.includes("네코 설정"); Object.entries(oldFields).forEach(([key, value]) => { fields[key].value = value; }); saveLoadWarning = oldWarning; return [memory.level, memoryGone, settingsClamped, settingsGone, warned]; })()', context);
if (chromeStorageGuard[0] !== 1 || !chromeStorageGuard[1] || !chromeStorageGuard[2] || !chromeStorageGuard[3] || !chromeStorageGuard[4]) throw new Error('Chrome 전용 네코 저장소 멈춤 방지 실패');
...
(async () => {
  ...
  await submit('정보구매 무한구역');
  if (!screenText().includes('[정보 구매]') || !screenText().includes('무한구역')) throw new Error('정보 구매 실패');
  await submit('품목');
  ...
  if (!elements.statusPanel.textContent.includes('무한구역 진입')) throw new Error('폐광 이후 무한구역 진입 임무 실패');
  await submit('이동 북문 밖 숲');
  await submit('이동 북문');
  await submit('이동 중앙광장');
  await submit('이동 주막');
  ...
  await submit('조사');
  if (!elements.statusPanel.textContent.includes('압축 좌표 조사')) throw new Error('무한구역 진입 이후 압축 좌표 임무 실패');
  if (!elements.statusPanel.textContent.includes('위치: 무한구역')) throw new Error('단일 무한구역 진입 실패');
  if (!elements.statusPanel.textContent.includes('[무한구역]') || !elements.statusPanel.textContent.includes('위상: 피의 위상')) throw new Error('무한구역 위상 상태 표시 실패');
  if (!screenText().includes('장면:') || !screenText().includes('적 전투력 x10') || !elements.statusPanel.textContent.includes('깨달음:')) throw new Error('무한구역 장면/10배 위험 표시 실패');
  if (!screenText().includes('장소 사건: 무한구역 감시자 단서')) throw new Error('장소 사건 조사 표시 실패');
  await submit('위상');
  if (!screenText().includes('[무한구역 위상]') || !screenText().includes('피의 위상')) throw new Error('무한구역 위상 명령 실패');
  await submit('전략 파밍');
  if (!elements.statusPanel.textContent.includes('자동 전략: 아이템 파밍')) throw new Error('자동 전략 변경 실패');
  await submit('계획');
  if (!screenText().includes('[네코 3턴 계획]') || !screenText().includes('전략: 아이템 파밍')) throw new Error('네코 3턴 계획 실패');
  await submit('사건');
  ...
  await submit('지도');
  if (!screenText().includes('[무한구역X x10]') || screenText().includes('10-10B')) throw new Error('무한구역 1구역 지도 표시 실패');
  await submit('조사');
  ...
  if (!elements.statusPanel.textContent.includes('자동 목표: 무한구역')) throw new Error('무한구역 자동 목표 변경 실패');
  await submit('자동');
  const frontierAutoStart = screenText();
  await context.autoTick();
  await context.autoTick();
  await context.autoTick();
  if (elements.statusPanel.textContent.includes('무한평원')) throw new Error('무한구역 자동 진행이 옛 좌표로 이동함');
  if (!screenText().slice(frontierAutoStart.length).includes('[자동 진행]')) throw new Error('무한구역 자동 진행 로그가 없음');
  await submit('자동');
  ...
  await context.autoTick();
  if (elements.statusPanel.textContent.includes('위치: 무한구역')) throw new Error('자동 진행 장소 2회 제한 이동 실패');
