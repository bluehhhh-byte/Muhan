...
const autoGambleRisk = vm.runInNewContext('(() => { const oldRoom = roomName; const oldGold = character.gold; const oldRisk = fields.risk.value; roomName = "도박장"; character.gold = 500; fields.risk.value = "공격"; const risky = bestAutoGambleChoice().command; fields.risk.value = "안전"; const safe = bestAutoGambleChoice().command; roomName = oldRoom; character.gold = oldGold; fields.risk.value = oldRisk; return [risky, safe]; })()', context);
if (!autoGambleRisk[0].includes('올인') || autoGambleRisk[1].includes('올인')) throw new Error('자동 도박 위험 성향 반영 실패');
const bankruptcyNote = vm.runInNewContext('(() => { const oldGold = character.gold; const oldMood = character.mood; const oldScene = character.lastScene; const oldInsight = character.insight; const oldContracts = economy.contracts.slice(); character.gold = 0; noteGambleBankruptcy("테스트", 100); const result = [character.mood, economy.contracts.includes("도박장 외상 주의")]; character.gold = oldGold; character.mood = oldMood; character.lastScene = oldScene; character.insight = oldInsight; economy.contracts = oldContracts; return result; })()', context);
if (bankruptcyNote[0] !== '절망' || !bankruptcyNote[1]) throw new Error('도박 파산 안내 실패');
