...
(async () => {
  ...
  await submit('사냥');
  await submit('사냥');
  if (elements.statusPanel.textContent.includes('레벨: 2')) throw new Error('전투 경험만으로 자동 레벨업됨');
  ...
  await submit('수련');
  if (!elements.statusPanel.textContent.includes('레벨: 2')) throw new Error('수련장 비용 지불 레벨업 실패');
  if (!screenText().includes('[수련 승급]') || !screenText().includes('지불했습니다')) throw new Error('수련 비용 지불 메시지가 없음');
  await submit('회복');
